...
#include "dbj++log/dbj++log.hpp"
#include "dbj++log/logworker.hpp"
#include "dbj++log/crashhandler.hpp"
#include "dbj++log/logmessage.hpp"
#include "dbj++log/loglevels.hpp"
         ...
         // Uninitialized messages are ignored but does not CHECK/crash the logger
		  if (!internal::isLoggingInitialized()) {
				  ...
				  std::cerr << "\nLOGGER NOT INITIALIZED: " << 
					  g_first_unintialized_msg->write() << std::endl;
		  }
		  else {
		  ...
		  /*
		  DBJ changed the bellow to the above
