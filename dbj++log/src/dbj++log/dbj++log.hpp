 ...
 * ============================================================================
 *
 * Filename:dbj++log.hpp  Framework for Logging and Design By Contract
...
#include "logworker.hpp"
 // #include "dbj++log/generated_definitions.hpp"
...
/*
----------------------------------------------------------------------------------
2019-03-11 DBJ: Added the default autoinitalization
                logging before main should be possible. This is how to check

				assert ( nullptr != dbj::pplog::g3log_default_initialized_log_file_name) ;

				if initialized that is the log file name.

----------------------------------------------------------------------------------
*/

#define G3LOG_PATH_TO_LOG_FILE "./"

namespace dbj::pplog {

	inline auto g3log_global_unique_worker = ::g3::LogWorker::createLogWorker();
		...
		= []() // -> char const *
	{
		...
		static std::string here_ = log_file_name.get();
		return here_.c_str();
	}();
}
