...
*
* For more information see dbj++log/LICENSE or refer refer to http://unlicense.org
* ============================================================================*/

#include "dbj++log/time.hpp"
...
namespace g3 {
	namespace internal {
		...
		//std::string localtime_formatted_fractions(
		//	const g3::system_time_point& ts,
		//	std::string format_buffer
		//);

		// not used --> constexpr inline auto date_formatted = "%Y/%m/%d"sv;
		...
		// in da header --> constexpr inline auto time_formatted = "%H:%M:%S %f6"sv;
		// in da header --> constexpr inline auto date_time_formatted = "%Y/%m/%d %H:%M:%S %f6"sv;
	} // internal
}
...
namespace g3 {
   namespace internal {
      ...
      constexpr auto kFractionalIdentier   = "%f"sv ;
      constexpr size_t kFractionalIdentierSize = kFractionalIdentier.size() ;

      Fractional getFractional(string_view format_buffer, size_t pos) {
         char  ch  = 
      ...
      std::string localtime_formatted_fractions
	  (const g3::system_time_point& ts, std::string format_buffer)
	  {
