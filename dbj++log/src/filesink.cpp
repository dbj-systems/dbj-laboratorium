...
namespace g3 {
   ...
   FileSink::~FileSink() {
      ...
      exit_msg.append(localtime_formatted
	  (now, internal::time_formatted.data() )
	  ).append("\n");
   ...
   std::string FileSink::changeLogFile(const std::string &directory, const std::string &logger_id) {
      ...
      auto now = std::chrono::system_clock::now();
      auto now_formatted 
		  = g3::localtime_formatted(now, {internal::date_time_formatted.data()});
